package com.example.androidcannongame;
...
public class CannonView extends SurfaceView implements SurfaceHolder.Callback {
					...
					cannonball.y - cannonballRadius < target.end.y)
			{
						...
						!hitStates[section])
				{
					...
					if (++targetPiecesHit == TARGET_PIECES)
					{
						...
						showNextLevelDialog(R.string.win); // show winning dialog
						gameOver = true; 
					} 
				}
			}
	...
	// display an AlertDialog when the player wins
	private void showNextLevelDialog(final int messageId)
	{
		// DialogFragment to display quiz stats and start new quiz
		final DialogFragment gameResult = 
				new DialogFragment()
	...
	public static void saveScore(Context context, int score, String playerName){
	    SharedPreferences sharedPreferences = context.getSharedPreferences("Cannon Game", Context.MODE_PRIVATE);
	...
	public void loadScore(Context context){
	    SharedPreferences sharedPreferences = context.getSharedPreferences("Cannon Game", Context.MODE_PRIVATE);
	    playerName = sharedPreferences.getString("PlayerName", "Anonymous");
